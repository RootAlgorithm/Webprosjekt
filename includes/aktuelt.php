<div class="aktuelt">
    <div class="aktuelt_top_holder">
        <div class="aktuelt_header">
            <div class="aktuelt_header_head"><h1>Aktuelt</h1></div>
            <div class="aktuelt_header_skille"></div>
            <div class="aktuelt_header_tekst"><p>Våre utvalgde for deg</p></div>
        </div>
        <div class="aktuelt_menu">
            <div class="aktuelt_menu_knapp"><p>Alle kategorier</p></div>
            <div class="aktuelt_menu_knapp"><p>Rabatter</p></div>
            <div class="aktuelt_menu_knapp"><p>Stillinger</p></div>
            <div class="aktuelt_menu_knapp"><p>Restauranter</p></div>
        </div>
    </div>
    <div class="aktuelt_innhold">
        <div class="aktuelt_innhold_card">
            <img class="aktuelt_innhold_card_bilde" src="Bilder/I_Naerheten/AktivitetBotaniskHage.png">
            <div class="aktuelt_innhold_card_head"><p>Aktiviteter</p></div>
            <div class="aktuelt_innhold_card_text"><p>Botianisk Hage</p></div>
        </div>
        <div class="aktuelt_innhold_card">
            <img class="aktuelt_innhold_card_bilde" src="Bilder/Studenter.jpg">
            <div class="aktuelt_innhold_card_head"><p>Stilling</p></div>
            <div class="aktuelt_innhold_card_text"><p>Øvingslærer PGR101</p></div>
        </div>
        <div class="aktuelt_innhold_card">
            <img class="aktuelt_innhold_card_bilde" src="Bilder/Paa_Capmus/SjakkForedning.jpg">
            <div class="aktuelt_innhold_card_head"><p>Elektronikk</p></div>
            <div class="aktuelt_innhold_card_text"><p>GamersGames.no</p></div>
        </div>
    </div>
</div>